...
public class ShovelDevilCommand extends BukkitCommand {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String @NotNull [] args) {
        ...
        if (!MCPrefixAchievement.getInstance().getUserPrefixManager().hasPrefix(player.getUniqueId(), PrefixName.SHOVEL_DEVIL)) {
            Prefix prefix = MCPrefixAchievement.getInstance().getPrefixConfigManager().getPrefixById(PrefixName.SHOVEL_DEVIL);
            if (prefix == null) {
                return false;
            }
        ...
        player.addPotionEffect(new PotionEffect(PotionEffectType.HASTE, PotionEffect.INFINITE_DURATION, 0));
        player.sendRichMessage("<bold>【 칭호 】 <reset><aqua>성급함 효과<reset>가 적용되었습니다 !");
