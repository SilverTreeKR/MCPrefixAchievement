...
public class TheThiefCommand extends BukkitCommand {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String @NotNull [] args) {
        ...
        if (!MCPrefixAchievement.getInstance().getUserPrefixManager().hasPrefix(player.getUniqueId(), PrefixName.THE_THIEF)) {
            Prefix prefix = MCPrefixAchievement.getInstance().getPrefixConfigManager().getPrefixById(PrefixName.THE_THIEF);
            if (prefix == null) {
                return false;
            }
        ...
        player.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 20*60*10, 0));
